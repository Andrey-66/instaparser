...
def create_post(instagram_post_id,
                profile_id,
                media_type,
                is_sent=False,
                sent_at=None,
    ...
    if response.status_code != 201:
        logger.error(f"Error creating post {instagram_post_id}")
        return False
    logger.info(f"Created post {instagram_post_id}")
    return True

def get_post(instagram_post_id):
    try:
        post = requests.get(f'{URL}/api/posts/{instagram_post_id}')
    except Exception as e:
        logger.error(f"Error getting post: {e}")
        return {}
    if post.status_code != 200:
        logger.error(f'Error getting profile: {post.status_code}')
        return {}
    return post.json()
